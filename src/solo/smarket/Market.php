...
class Market {
    ...
    public function __construct(int $id, Item $item, $buyPrice = -1, $sellPrice = -1) {
        $this->id = $id;
        $this->item = Item::get($item->getId(), $item->getDamage(), 1, $item->getNamedTag());
    ...
    public function updateTile(Tile $tile) {
        if ($tile instanceof Sign) {
            ...
            if ($display->getId() !== $this->item->getId() or $display->getDamage() !== $this->item->getDamage() or $display->getCustomName() !== $text) {
                $display = (clone $this->item)->setCount(1);
    ...
    public function jsonSerialize() {
        return [
                ...
                "item" => [
                        ...
                        "nbt_b64" => base64_encode($this->executeMethod("writeCompoundTag")->invoke($this->item, $this->item->getNamedTag()))
                ],
